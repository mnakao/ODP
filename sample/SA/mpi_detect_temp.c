...
double calc_min_temp_mpi_s()
{
  return -2.0 / log(0.0001);
}

double calc_min_temp_mpi()
{
  return calc_min_temp_mpi_s();
}
...
double calc_max_temp_mpi_s(const int nodes, const int degree, const int seed, const MPI_Comm comm, const int symmetries)
{
  ...
  ODP_Init_aspl_mpi_general_s(nodes, degree, NULL, comm, symmetries);
...
double calc_max_temp_mpi(const int nodes, const int degree, const int seed, MPI_Comm comm)
{
  return calc_max_temp_mpi_s(nodes, degree, seed, comm, 1);
}
