...
{
  ERROR("%s -L length -S symmetries [-W width] [-H height] [-D degree] [-o output] [-s seed] [-n calcs] [-w max_temp] [-c min_temp] [-Y] [-A]\n", argv);
}

static void set_args(const int argc, char **argv, int *width, int *height, int *degree, int *length, int *symmetries, char **infname,
                     char **outfname, int *seed, long *ncalcs, double *max_temp, double *min_temp, bool *hill_climbing, bool *ASPL_priority)
{
  int result;
  while((result = getopt(argc,argv,"W:H:D:L:S:f:o:s:n:w:c:YA"))!=-1){
    switch(result){
    case 'W':
      *width = atoi(optarg);
      if(*width <= 0)
...
int main(int argc, char *argv[])
{
  ...
  set_args(argc, argv, &width, &height, &degree, &length, &symmetries, &infname,
           &outfname, &seed, &ncalcs, &max_temp, &min_temp, &hill_climbing, &ASPL_priority);
