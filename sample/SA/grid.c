...
{
  ERROR("%s -L length [-W width] [-H height] [-D degree] [-f input] [-o output] [-s seed] [-n calcs] [-w max_temp] [-c min_temp] [-A]\n", argv);
}
...
{
  int result;
  while((result = getopt(argc,argv,"W:H:D:L:f:o:s:n:w:c:A"))!=-1){
    switch(result){
    case 'W':
      *width = atoi(optarg);
      if(*width <= 0)
