...
int main(int argc, char *argv[])
{
  ...
  if(diameter == low_diameter && ASPL == low_ASPL){
    ...
    for(long i=0;i<ncalcs;i++){
      ...
      int u[4], v[4], u_d[4], v_d[4], new_u[4], new_v[4];
      while(1){
        while(1){
          u[0] = get_random(nodes);
          u[1] = get_random(nodes);
          u[2] = get_random(nodes);
          u[3] = get_random(nodes);
          if(u[0] == u[1] || u[0] == u[2] || u[0] == u[3] || u[1] == u[2] || u[1] == u[3] || u[2] == u[3]) continue;

          u_d[0] = get_random(degree);
          v[0]   = adjacency[u[0]][u_d[0]];
          if(v[0] == u[1] || v[0] == u[2] || v[0] == u[3]) continue;
          
          u_d[1] = get_random(degree);
          v[1]   = adjacency[u[1]][u_d[1]];
          if(v[1] == u[0] || v[1] == v[0] || v[1] == u[2] || v[1] == u[3]) continue;

          u_d[2] = get_random(degree);
          v[2]   = adjacency[u[2]][u_d[2]];
          if(v[2] == u[0] || v[2] == u[1] || v[2] == v[0] || v[2] == v[1] || v[2] == u[3]) continue;

          u_d[3] = get_random(degree);
          v[3]   = adjacency[u[3]][u_d[3]];
          if(v[3] == u[0] || v[3] == u[1] || v[3] == u[2] || v[3] == v[0] || v[3] == v[1] || v[3] == v[2]) continue;
          break;
        }
        ...
        v_d[3] = get_degree_index(u[3], v[3], u_d[3], nodes, degree, (const int (*)[degree])adjacency);
        backup_adjacency(4, u, u_d, v, v_d);

        // 4-opt: u[0]--u[1], v[0]--u[2], v[1]--u[3], v[2]--v[3]
        new_u[0] = u[2]; new_u[1] = u[3]; new_u[2] = v[3]; new_u[3] = v[2];
        new_v[0] = u[1]; new_v[1] = u[0]; new_v[2] = v[0]; new_v[3] = v[1];
        ...
        if(!check_multiple_edges(u[0], u_d[0], new_v[0], degree, (const int (*)[degree])adjacency) ||
           ...
           !check_multiple_edges(u[3], u_d[3], new_v[3], degree, (const int (*)[degree])adjacency) ||
	...
	current_ASPL     = ASPL;
        ...
        undo_adjacency(4, degree, adjacency);
      }
      temp *= cooling_rate;
    }
  }
