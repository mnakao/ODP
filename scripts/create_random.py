...
def main(args):
	...
	degree = args.degree
        ...
        if args.l:
            ...
            width  = args.W
            heigth = args.H
            assert nodes == width * heigth

	assert degree < nodes
	
	low_diam, low_aspl = lower_bound_of_diam_aspl(nodes, degree)
	g = nx.random_regular_graph(degree, nodes, 0)
	if nx.is_connected(g):
        ...
        if args.l:
            ...
            outfname = "n{}d{}.random".format(nodes, degree) + ".edges"
...
def lower_bound_of_diam_aspl(nodes, degree):
	diam = -1
	aspl = 0.0
	n = 1
	r = 1
