...
static __global__ void init_buffers(uint64_t* __restrict__ A, uint64_t* __restrict__ B, const int nodes,
				    const int symmetries, const int t, const int chunk, const int height)
{
			      ...
			      int *diameter, long *sum, double *ASPL)
{
  ...
  *diameter = 1;
  ...
  for(int t=_rank;t<parsize;t+=_procs){
    ...
    init_buffers      <<< BLOCKS, THREADS >>> (_A_dev, _B_dev, _nodes, _symmetries, t, chunk, _height);

    for(kk=0;kk<_nodes;kk++){
      ODP_Matmul_cuda <<< BLOCKS, THREADS >>> (_A_dev, _B_dev, _nodes, _height, _degree, _num_degrees_dev,
					       _adjacency_dev, _itable_dev, chunk, _symmetries);
				     ...
				     int *diameter, long *sum, double *ASPL)
{
  ...
  *diameter = 1;
  ...
  for(int t=_rank;t<parsize;t+=_procs){
    ...
    init_buffers      <<< BLOCKS, THREADS >>> (_A_dev, _B_dev, _nodes, _symmetries, t, GPU_CHUNK, _height);
				 ...
				 const MPI_Comm comm, const int symmetries)
{
  ...
  _kind = ODP_Get_kind(nodes, degree, num_degrees, symmetries, _procs, false, _enable_grid_s);
  _mem_usage = ODP_Get_mem_usage(_kind, nodes, degree, symmetries, num_degrees, _procs, false, _enable_grid_s);
